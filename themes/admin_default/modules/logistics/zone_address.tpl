	...
	<div class="row">
		...
		<div class="col-xs-24 col-sm-12 col-md-6">
			<select name="id_zone" class="form-control">
				...
				<!-- BEGIN: zone_search -->
				<option {selected} value="{zone.id}">{zone.title}</option>
				<!-- END: zone_search -->
			</select>
		</div>
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
	...
	<div class="form-group">
		...
		<div class="col-sm-19 col-md-20">
			<select name="district[]" multiple="multiple" class="form-control quanhuyen">
	...
	<div class="form-group">
		...
		<div class="col-sm-19 col-md-20">
			<select name="wards[]" multiple="multiple" class="form-control xaphuong">
											<option value="0">-- Chọn quận huyện --</option>
											<!-- BEGIN: xa -->
...
<script type="text/javascript">
	...
	$('.tinhthanh').change(function(){
		...
		if(tinhthanh > 0)
		{
				$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone_address&id_tinhthanh=' + tinhthanh, function(res) {
					$('.quanhuyen').html(res);
	...
	$('select.quanhuyen').change(function(){
		...
		if(id_quanhuyen != '')
		{
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone_address&id_quanhuyen=' + id_quanhuyen, function(res) {
					$('.xaphuong').html(res);
...
//<![CDATA[
	function nv_change_weight(id) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone_address&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] != 'OK') {
				alert(nv_is_change_act_confirm[2]);
			}
			window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone_address';
			return;
		});
		return;
	}
	...
	function nv_change_status(id) {
		...
		if (confirm(nv_is_change_act_confirm[0])) {
			...
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone_address&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
				var r_split = res.split('_');
				if (r_split[0] != 'OK') {
					alert(nv_is_change_act_confirm[2]);
				}
