...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'status = 0 AND send_name LIKE :q_send_name OR send_phone LIKE :q_send_phone OR receive_name LIKE :q_receive_name OR receive_phone LIKE :q_receive_phone OR document_name LIKE :q_document_name OR bill LIKE :q_bill OR value_goods LIKE :q_value_goods OR id_service LIKE :q_id_service' );
	}
	else
	{
		$db->where( 'status = 0');
