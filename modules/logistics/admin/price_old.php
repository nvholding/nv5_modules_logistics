...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			if( empty( $row['id'] ) )
			{
				$stmt = $db->prepare( 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_price (weight, quantity, id_document, id_service, id_zone, active, price, note) VALUES (:weight, :quantity, :id_document, :id_service, :id_zone, :active, :price, :note)' );
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_price SET quantity = :quantity, id_document = :id_document, id_service = :id_service, id_zone = :id_zone, price = :price, note = :note WHERE id=' . $row['id'] );
			}
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'quantity LIKE :q_quantity '. $where );
	}
	else
	{
		$db->where( 'id > 0 '. $where );
