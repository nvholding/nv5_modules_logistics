...
if ( $nv_Request->isset_request( 'delete_id', 'get' ) and $nv_Request->isset_request( 'delete_checkss', 'get' ))
{
	...
	{
		$weight=0;
		$sql = 'SELECT weight FROM ' . $db_config['prefix'] . '_' . $module_data . '_store WHERE id =' . $db->quote( $id );
		$result = $db->query( $sql );
		list( $weight) = $result->fetch( 3 );
		
		$db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_store  WHERE id = ' . $db->quote( $id ) );
		if( $weight > 0)
		{
...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_store SET title = :title, phone = :phone, name =:name, city = :city, district = :district, wards = :wards, address = :address, note = :note, userid =:userid WHERE id=' . $row['id'] );
			}
			...
			{
				$nv_Cache->delMod( $module_name );
				Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=store');
				die();
			}
		}
...
// LẤY DANH TRẠNG THÁI ĐƠN HÀNG
	...
	foreach($list_user as $user)
	{
		...
		$xtpl->assign( 'selected', $user['userid'] == $row['userid'] ? 'selected=selected' : '' );
