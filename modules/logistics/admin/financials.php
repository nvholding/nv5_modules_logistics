...
$base_url_lichtrinh = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op;

$where = '';
$where_lichtrinh = '';
$bill = $nv_Request->get_title( 'bill', 'post,get' );

$userid = $nv_Request->get_int( 'userid', 'post,get' );
$id_store = $nv_Request->get_int( 'id_store', 'post,get' );
...
if($id_store > 0)
{
	...
	$base_url_lichtrinh .= '&id_store=' . $id_store;
}
...
if( $show_view )
{
	...
	if($export_excel == 1)
	{
		...
		$sth_excel->execute();
		$num_items = $sth_excel->fetchColumn();

		$db->select('t2.add_date as ngay, t1.*')
			->order( 't2.add_date DESC' );
		$sth_excel = $db->prepare( $db->sql() );
		//die($db->sql());
		$sth_excel->execute();
	...
	$excel->getActiveSheet()->getStyle('A1:L1')->getFont()->setBold(true);
		...
		while( $view = $sth_excel->fetch() )
		{
			...
			$excel->getActiveSheet()->setCellValue('C'.$numRow, date('d/m/Y - H:i',$view['ngay']));
