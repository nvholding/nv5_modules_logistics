...
if($id > 0)
{
	$array_data = $db->query(" SELECT * FROM ". $db_config['prefix'] . "_" . $module_data . "_rows WHERE id =".$id)->fetch();
}
elseif(!empty($array_op[1]) and empty($array_data))
{
	// LẤY RA ID VẬN ĐƠN
...
$contents = nv_theme_logistics_print( $array_data, $list_document, $list_service, $list_surcharge );
