...
$xtpl->assign( 'FILE_EXCEL', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload  . '/File_Excel.xlsx');  





$id_store = $nv_Request->get_int('store','post,get', 0);
  ...
  $allowedExts = array("xlsx");
    ...
    if (($_FILES["excel"]["size"] < 200000000000) && in_array($extension, $allowedExts)) {
        if ($_FILES["excel"]["error"] > 0)
			...
			$filename = NV_UPLOADS_DIR . '/' . $module_upload . '/' . $user_info['username'];

			if  (!file_exists($filename)) {
				mkdir(NV_UPLOADS_DIR . '/' . $module_upload .  '/' . $user_info['username'], 0777);
			} 
