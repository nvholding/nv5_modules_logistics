...
if($nv_Request->isset_request('wards_cus', 'get'))
{
	$city_cus = $nv_Request->get_int('city_cus','get', 0);
	$district_cus = $nv_Request->get_int('district_cus','get', 0);
	$wards_cus = $nv_Request->get_int('wards_cus','get', 0);
	$cus = array();
	$cus['city_cus'] = '';
	$cus['district_cus'] = '';
	$cus['wards_cus'] = '';
	...
	if($city_cus > 0)
	{
		...
		$html_tinhthanh = '<option value=0>-- Chọn tỉnh thành --</option>';
					foreach($list_tinhthanh as $l)
					{
						if($l['provinceid'] == $city_cus)
						$selected = 'selected=selected';
						else $selected = '';
						$html_tinhthanh .= '<option '. $selected .' value='.$l['provinceid'].'>'.$l['type'] . ' '. $l['title'].'</option>';
					}
		$cus['city_cus'] = $html_tinhthanh;
	}
	
	if($district_cus > 0 and $city_cus > 0)
	{
		$list_quan = $db->query('SELECT * FROM tms_location_district WHERE status = 1 and provinceid = '. $city_cus .' ORDER BY weight ASC')->fetchAll();
		$html_quan = '<option value=0>-- Chọn quận huyện --</option>';
					foreach($list_quan as $l)
					{
						if($l['districtid'] == $district_cus)
						$selected = 'selected=selected';
						else $selected = '';
						$html_quan .= '<option '. $selected .' value='.$l['districtid'].'>'.$l['type'] . ' '. $l['title'].'</option>';
					}
		$cus['district_cus'] = $html_quan;
	}
	
	if($wards_cus > 0 and $district_cus > 0)
	{
		$list_xaphuong = $db->query('SELECT wardid, title ,type FROM tms_location_ward WHERE districtid = '. $district_cus .' and status = 1')->fetchAll();
		$html_xaphuong = '<option value=0>-- Chọn xã phường --</option>';
					foreach($list_xaphuong as $l)
					{
						if($l['wardid'] == $wards_cus)
						$selected = 'selected=selected';
						else $selected = '';
						$html_xaphuong .= '<option '. $selected .' value='.$l['wardid'].'>'.$l['type'] . ' '. $l['title'].'</option>';
					}
		$cus['wards_cus'] = $html_xaphuong;
	}
	
	$myJSON = json_encode($cus);
...
if($nv_Request->isset_request('id_service_a', 'post'))
{
	...
	$tien_hang = $nv_Request->get_float('tien_hang','post', 0);
	...
	if($id_service > 0 and $id_document > 0 and $send_city > 0 and $send_district > 0 and $weight_document > 0)
	{
		...
		if($id_khuvuc > 0)
		{
			...
			// Thông tin cước phí khu vực
			
			$cuoc_nhap = $db->query('SELECT entry_fee, import, configuration FROM '.$db_config['prefix'] . '_' . $module_data . '_zone WHERE id ='.$id_khuvuc)->fetch();
			$price['kv'] = $cuoc_nhap;
			$list_configuration = explode(',',$cuoc_nhap['configuration']);
		}
		
		// % CƯỚC TÀI LIỆU
		if($cuoc_nhap['entry_fee'] == 2 and in_array($id_document,$list_configuration) )
		{
			$cuoc_tailieu = $db->query('SELECT price, price_min FROM '.$db_config['prefix'] . '_' . $module_data . '_price_document WHERE id_document ='.$id_document)->fetch();
			$array_cuoc_tailieu = json_decode($cuoc_tailieu['price'],true);
			
			$price['charge_for_collection'] == 0;
			
			if(count($array_cuoc_tailieu) == 1)
			{
				$price['charge_for_collection'] = $array_cuoc_tailieu[0]['price'];
			}
			else
			{
				foreach($array_cuoc_tailieu as $row)
				{
					if(!empty($row['quantity']))
					{
						...
						if($tien_hang >= $mang_khoiluong[0] and $tien_hang <= $mang_khoiluong[1])
						{
							$price['charge_for_collection'] = $row['price'];
						}
		...
		elseif($cuoc_nhap['import'] > 0)
		{
			$price['charge_for_collection'] = $cuoc_nhap['import'];
		}
		
		
		$price['cuoc_phi_hang'] = 0;
		
		if($tien_hang >  0)
		{
			$price['cuoc_phi_hang'] = $tien_hang * ($price['charge_for_collection'] / 100);
		}
		
		// CƯỚC PHÍ THẤP NHẤT
		if($price['cuoc_phi_hang'] > 0)
		{
			if($price['cuoc_phi_hang'] < $cuoc_tailieu['price_min'] and $cuoc_nhap['entry_fee'] == 2)
			{
				$price['cuoc_phi_hang'] = $cuoc_tailieu['price_min'];
			}
			elseif($price['cuoc_phi_hang'] < 15000)
				$price['cuoc_phi_hang'] = 15000;
		}
...
if($nv_Request->isset_request('customer_phone', 'post'))
{
	...
	if($customer_userid == 0)
		$customer_userid = $user_info['userid'];
...
// KIỂM TRA TÀI KHOẢN ĐÃ TẠO KHO CHƯA

$dem_kho = $db->query('SELECT COUNT(id) FROM ' . $db_config['prefix'] . '_' . $module_data . '_store WHERE userid ='. $user_info['userid'] . ' AND status = 1')->fetchColumn();
if($dem_kho == 0)
{
	Header( 'Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=store' );
    die();
}
	...
	if( empty( $error ) )
	{
		...
		try
		{
			if( empty( $row['id'] ) )
			{
				...
				$row['userid_add'] = $user_info['userid'];
		...
		// KẾT THÚC THÊM KHÁCH HÀNG
				...
				if( empty( $row['id'] ) )
				{
					...
					$id_moi_tao = $db->query( 'SELECT max(id) FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE userid_add ='.$user_info['userid'] )->fetchColumn();
				...
				$nv_Cache->delMod( $module_name );
				Header( 'Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=items' );
				die();
			}
		}
		catch( PDOException $e )
...
$xtpl->assign( 'Q', $q );
	...
	$list_store = $db->query('SELECT * FROM '.$db_config['prefix'] . '_' . $module_data . '_store WHERE status = 1 AND userid ='.$user_info['userid'].' ORDER BY weight ASC')->fetchAll();
