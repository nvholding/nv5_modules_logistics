...
// LẤY LỊCH TRÌNH CUỐI CÙNG RA
$status = $db->query('SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_schedule WHERE status = 1 ORDER BY weight DESC LIMIT 0,1')->fetchColumn();
...
if( $store_s > 0 )
{
	...
	$base_url_lichtrinh .= '&store=' . $store_s;
}
...
if(!empty($userid))
{
	...
	$base_url_lichtrinh .= '&userid=' . $userid;
}
...
if($ngay_tu > 0 and $ngay_den > 0)
{
	...
	$where .=" AND t2.add_date >= ". $ngay_tu . " AND t2.add_date <= ". $ngay_den;
	$base_url .= '&ngay_tu=' . date('d/m/Y',$ngay_tu) .'&ngay_den='.date('d/m/Y',$ngay_den);
}
elseif( $ngay_tu > 0 )
{
	$where .=" AND t2.add_date >= ". $ngay_tu;
	$base_url .= '&ngay_tu=' . date('d/m/Y',$ngay_tu) .'&ngay_den='.date('d/m/Y',$ngay_den);
}
elseif( $ngay_den > 0 )
{
	$where .=" AND t2.add_date <= ". $ngay_den;
	$base_url .= '&ngay_tu=' . date('d/m/Y',$ngay_tu) .'&ngay_den='.date('d/m/Y',$ngay_den);
}
else
{
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	$export_excel = $nv_Request->get_int( 'export_excel', 'post, get', 0 );
	...
	if($export_excel == 1)
	{
		...
		$db->select('DISTINCT(t1.id), t2.add_date as ngay, t1.*')
			->order( 'weight ASC' );
		$sth_excel = $db->prepare( $db->sql() );
		//die($db->sql());
		$sth_excel->execute();
	...
	$excel->getActiveSheet()->setTitle('Báo cáo tài chính');
	...
	$excel->getActiveSheet()->getStyle('A1:L1')->getFont()->setBold(true);
		...
		$excel->getActiveSheet()->setCellValue('B1', $lang_module['bill']);
		$excel->getActiveSheet()->setCellValue('C1', $lang_module['add_date_doisoat']);
		$excel->getActiveSheet()->setCellValue('D1', $lang_module['id_service']);
		$excel->getActiveSheet()->setCellValue('E1', $lang_module['value_goods']);
		$excel->getActiveSheet()->setCellValue('F1', $lang_module['money_collection']);
		$excel->getActiveSheet()->setCellValue('G1', $lang_module['service_charge']);
		$excel->getActiveSheet()->setCellValue('H1', $lang_module['pays']);
		$excel->getActiveSheet()->setCellValue('I1', $lang_module['total_charge']);
		$excel->getActiveSheet()->setCellValue('J1', $lang_module['vat']);
		$excel->getActiveSheet()->setCellValue('K1', $lang_module['total_money']);
		
	
		$stt = 1;
		$numRow = 2;
		
		$phithuho = $tienthuho = $cuocdichvu = $dichvugiatang = $cuoctamtinh = $vat = $tongcuocthu = 0 ;
		...
		while( $view = $sth_excel->fetch() )
		{
			...
			$phithuho = $phithuho + $view['charge_for_collection'];
			$tienthuho = $tienthuho + $view['money_collection'];
			$cuocdichvu = $cuocdichvu + $view['service_charge'];
			$dichvugiatang = $dichvugiatang + $view['pays'];
			$cuoctamtinh = $cuoctamtinh + $view['total_charge'];
			$vat = $vat + $view['vat'];
			$tongcuocthu = $tongcuocthu + $view['total_money'];
			...
			$view['userid_add'] = $db->query('SELECT username FROM '. $db_config['prefix'] .'_users WHERE userid = '. $view['userid_add'])->fetchColumn();
						
			$excel->getActiveSheet()->setCellValue('A'.$numRow, $stt);
			$excel->getActiveSheet()->setCellValue('B'.$numRow, $view['userid_add'] . ' - ' . $view['bill']);
			$excel->getActiveSheet()->setCellValue('C'.$numRow, date('d/m/Y',$view['ngay']));
			$excel->getActiveSheet()->setCellValue('D'.$numRow, $view['id_service']);
			$excel->getActiveSheet()->setCellValue('E'.$numRow, $view['money_collection']);
			$excel->getActiveSheet()->setCellValue('F'.$numRow, $view['charge_for_collection']);
			$excel->getActiveSheet()->setCellValue('G'.$numRow, $view['service_charge']);
			$excel->getActiveSheet()->setCellValue('H'.$numRow, $view['pays']);
			$excel->getActiveSheet()->setCellValue('I'.$numRow, $view['total_charge']);
			$excel->getActiveSheet()->setCellValue('J'.$numRow, $view['vat']);
			$excel->getActiveSheet()->setCellValue('K'.$numRow, $view['total_money']);
			$numRow++;
			$stt++;
		...
		$tmp = '<html>
                    ...
                    <body><div style="text-align:center;font-size:20px;color:red">TỔNG THU</div></body></html>';
		...
		$excel->getActiveSheet()->setCellValue('A'.$numRow, $wizard->toRichTextObject($tmp));
		$excel->getActiveSheet()->setCellValue('E'.$numRow, $tienthuho);
		$excel->getActiveSheet()->setCellValue('F'.$numRow, $phithuho);
		$excel->getActiveSheet()->setCellValue('G'.$numRow, $cuocdichvu);
		$excel->getActiveSheet()->setCellValue('H'.$numRow, $dichvugiatang);
		$excel->getActiveSheet()->setCellValue('I'.$numRow, $cuoctamtinh);
		$excel->getActiveSheet()->setCellValue('J'.$numRow, $vat);
		$excel->getActiveSheet()->setCellValue('K'.$numRow, $tongcuocthu); 
		
		//$excel->getActiveSheet()->getStyle('J'.$numRow, $vat)->applyFromArray($money)->getNumberFormat()->setFormatCode('#,##0');
		//$excel->getActiveSheet()->getStyle('K'.$numRow, $tongcuocthu)->applyFromArray($money)->getNumberFormat()->setFormatCode('#,##0'); 
...
if( $show_view )
{
	...
	while( $view = $sth->fetch() )
	{
		...
		$phithuho = $phithuho + $view['money_collection'];
		$tienthuho = $tienthuho + $view['charge_for_collection'];
		$cuocdichvu = $cuocdichvu + $view['service_charge'];
		$dichvugiatang = $dichvugiatang + $view['pays'];
		$cuoctamtinh = $cuoctamtinh + $view['charge_for_collection'];
		$vat = $vat + $view['vat'];
		$tongcuocthu = $tongcuocthu + $view['total_money'];
		...
		$view['add_date'] = date('d/m/Y - h:i', $view['ngay']);
		$stt++;
		$xtpl->assign( 'stt', $stt );
		...
		$view['money_collection'] = number_format($view['money_collection'],0,",",",");
		$view['service_charge'] = number_format($view['service_charge'],0,",",",");
		$view['pays'] = number_format($view['pays'],0,",",",");
		$view['charge_for_collection'] = number_format($view['charge_for_collection'],0,",",",");
		$view['total_charge'] = number_format($view['total_charge'],0,",",",");
		$view['vat'] = number_format($view['vat'],0,",",",");
	...
	$xtpl->assign( 'phithuho', number_format($phithuho,0,",",",") );
	$xtpl->assign( 'tienthuho', number_format($tienthuho,0,",",",") );
	$xtpl->assign( 'cuocdichvu', number_format($cuocdichvu ,0,",",","));
	$xtpl->assign( 'dichvugiatang', number_format($dichvugiatang,0,",",",") );
	$xtpl->assign( 'cuoctamtinh', number_format($cuoctamtinh,0,",",",") );
	$xtpl->assign( 'vat', number_format($vat,0,",",",") );
	$xtpl->assign( 'tongcuocthu', number_format($tongcuocthu ,0,",",","));
